...
async function main() {
  try {
    ...
    console.log('GeoServer REST version info', prettyJson(await grc.about.getVersion()));
    ...
    console.log('Created GeoServer NS', await grc.namespaces.create(nsPrefix, nsUri));

    console.log('GeoServer All NS', prettyJson(await grc.namespaces.getAll()));

    console.log('GeoServer NS', prettyJson(await grc.namespaces.get(nsPrefix)));

    console.log('Deleted GeoServer NS', await grc.namespaces.delete(nsPrefix));

    // WORKSPACES;

    console.log('GeoServer All WS', prettyJson(await grc.workspaces.getAll()));

    console.log('Created GeoServer WS', prettyJson(await grc.workspaces.create(ws)));

    const recursive = true;
    console.log('Deleted GeoServer WS', await grc.workspaces.delete(ws, recursive));

    // DATASTORES;

    console.log(
      'GeoServer data stores in WS',
      ws,
      prettyJson(await grc.datastores.getDataStores(ws))
    );

    console.log(
      'GeoServer coverage stores in WS',
      ws,
      prettyJson(await grc.datastores.getCoverageStores(ws))
    );

    console.log(
      'GeoServer WMS stores in WS',
      ws,
      prettyJson(await grc.datastores.getWmsStores(ws))
    );

    console.log(
      'GeoServer WMTS stores in WS',
      ws,
      prettyJson(await grc.datastores.getWmtsStores(ws))
    );

    console.log('GeoServer data store ', prettyJson(await grc.datastores.getDataStore(ws, 'foo')));
    ...
    console.log('GeoServer wms store ', prettyJson(await grc.datastores.getWmsStore(ws, 'foo')));
    ...
    console.log('Deleted data store', await grc.datastores.deleteDataStore(ws, 'testWfsDs', false));
    ...
    console.log('All GeoServer layers', prettyJson(await grc.layers.getAll()));

    console.log(
      'Created Layer',
      prettyJson(
        await grc.layers.publishFeatureTypeDefaultDataStore(
          ws,
          'nativeFtName',
          'aTestName',
          'Test Title',
    ...
    console.log(prettyJson(await grc.layers.getFeatureType(ws, 'testdataStore', 'testName')));
    ...
    console.log(
      ...
      await grc.layers.publishDbRaster(
        ws,
        'testdataStore',
        'nativeRasterName',
        'a-db-raster',
        'Dummy DB Raster Title',
        null,
        true
      )
    );
    ...
    console.log(
      ...
      await grc.layers.enableTimeCoverage(
        ws,
        'testDs',
        'HymptyLayer',
        'DISCRETE_INTERVAL',
    ...
    console.log('Get Layers', prettyJson(await grc.layers.getLayers('my-workspace')));
    ...
    console.log('GeoServer default styles', prettyJson(await grc.styles.getDefaults()));

    console.log('GeoServer styles for WS', prettyJson(await grc.styles.getInWorkspace(ws)));

    console.log('GeoServer all WS styles', prettyJson(await grc.styles.getAllWorkspaceStyles()));

    console.log('GeoServer all styles', prettyJson(await grc.styles.getAll()));

    const sldBody =
      '<?xml version="1.0" encoding="UTF-8"?>\n<StyledLayerDescriptor version="1.0.0" \n xsi:schemaLocation="http://www.opengis.net/sld StyledLayerDescriptor.xsd" \n xmlns="http://www.opengis.net/sld" \n xmlns:ogc="http://www.opengis.net/ogc" \n xmlns:xlink="http://www.w3.org/1999/xlink" \n xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">\n  <NamedLayer>\n    <Name>default_line</Name>\n    <UserStyle>\n  <Title>Default Line</Title>\n      <Abstract>A sample style that draws a line</Abstract>\n   <FeatureTypeStyle>\n        <Rule>\n          <Name>rule1</Name>\n          <Title>Blue Line</Title>\n          <Abstract>A solid blue line with a 1 pixel width</Abstract>\n          <LineSymbolizer>\n            <Stroke>\n              <CssParameter name="stroke">#0000FF</CssParameter>\n            </Stroke>\n          </LineSymbolizer>\n        </Rule>\n      </FeatureTypeStyle>\n    </UserStyle>\n  </NamedLayer>\n</StyledLayerDescriptor>\n';
    console.log('Publish GeoServer style', await grc.styles.publish(ws, 'testStyle3', sldBody));
    ...
    console.log(
      ...
      prettyJson(
        await grc.imagemosaics.harvestGranules(
          'imagemosaic_test',
          'testCovStore',
          filePathCoverages
        )
